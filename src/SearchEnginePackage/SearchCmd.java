    	...
    	while(line != null) {
            if(isPage(line)) {
                ...
                if(exists(hashMap, line)) {
                    ...
                    URLList duplicateURLEntry = find(hashMap.get(line.hashCode()), lastURL); 
    ...
    public static BinarySearchTree buildBST(String filename) throws IOException {
    	...
    	while(line != null) {
            if(isPage(line)) {
                ...
                if(exists(bst, line)) {
                    // Check for this url already exisiting with this name
                    URLList duplicateURLEntry = find(bst.find(line.hashCode(), bst.root).value, lastURL);
                    // If it is not a duplicate we update the hashMap
                    if(duplicateURLEntry == null) { 
...
public class SearchCmd {
    ...
    public static String processInput(String line, BPlusTree bTree) {
    	...
    	return processSingleQuery(line, bTree); // We have a single query
    }
